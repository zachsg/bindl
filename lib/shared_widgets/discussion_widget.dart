...
class _DiscussionWidgetState extends ConsumerState<DiscussionWidget> {
        ...
        children: [
          Expanded(
            child: meal.comments.isEmpty
                ? _emptyState()
                : _conversationList(meal, context),
          ),
          Padding(
            padding: const EdgeInsets.all(16.0),
            child: _sendMessageBox(meal, context),
          ),
        ],
      ),
    );
  ...
  SingleChildScrollView _conversationList(Meal meal, BuildContext context) {
  ...
  Row _sendMessageBox(Meal meal, BuildContext context) {
    return Row(
      children: [
        Expanded(
          child: TextField(
        ...
        IconButton(
          onPressed: () async {
            if (_textController.text.isNotEmpty) {
              ...
              await ref.read(mealsProvider.notifier).addComment(meal, message);
